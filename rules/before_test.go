...
var beforeRuleTestData = map[string]any{
	...
	"notBefore": map[string]any{
		...
		"output": map[string]any{
			...
			"validationError":  "The field start must be before field end.",
		},
	},
...
func TestBeforeRule(t *testing.T) {
	...
	for name, d := range beforeRuleTestData {
...
func initBeforeRule() *Before {
	beforeRule := &Before{}
	beforeRule.AddTranslationFunction(func(_, key string, params ...map[string]string) string {
		...
		case "validation.before":
			tr := "The field :field: must be before field :otherField:."
			for k, v := range p {
...
func TestBefore_MinRequiredParams(t *testing.T) {
	rule := initBeforeRule()
